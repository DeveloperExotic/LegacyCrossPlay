...
function sendAddMobPacket(proxy, client, entityInfo) {
  try {
    ...
    const posX = Math.floor(entityInfo.x * 32);
...
function sendAddEntityPacket(proxy, client, entityInfo) {
  try {
    if (!entityInfo || entityInfo.entityId === undefined) {
      return;
    }
    ...
    const yaw = Number.isFinite(entityInfo.yaw) ? entityInfo.yaw : 0;
    const pitch = Number.isFinite(entityInfo.pitch) ? entityInfo.pitch : 0;

    const yRotByte = Math.floor(((yaw % 360) / 360) * 256) & 0xff;
    const xRotByte = Math.floor(((pitch % 360) / 360) * 256) & 0xff;
    ...
    if (!buffer || buffer.length === 0 || buffer.length > 1000) {
      return;
    }

    proxy.sendPacket(client, 0x17, buffer);
  } catch (err) {
    /* do nothing */
  }
}
